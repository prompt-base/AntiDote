...
try:
    ...
    # If alzy_utils isn't present, define minimal stubs so page doesn't crash.
    # Ideally you should have the full alzy_utils.py in ANTIDOTE/utils/
    def load_data():
        return {
            "profile": {"name": "Friend"},
            "reminders": {},
...
# If you want to preserve quiz state across reruns
if "quiz_target_id" not in st.session_state:
    st.session_state.quiz_target_id = None
    st.session_state.quiz_option_ids = []

# allow ?say=... param for speech-to-text captured in browser JS
spoken_qp = st.experimental_get_query_params().get("say", None)
if isinstance(spoken_qp, list):
    spoken_qp = spoken_qp[0]
...
with right:
    ...
    if st.button("🔁 Back to role selection"):
        # safe switch: send user back to landing page
        try:
...
with tab_mbook:
    ...
    st.caption(f"Looking in: {(Path(__file__).resolve().parents[1] / 'uploads') if 'uploads' else 'uploads'}")
...
with tab_gps:
    ...
    if st.button("🧭 Go home"):
        # Open Google directions in new window
...
with tab_ai:
    ...
    # if spoken text present in URL param, prefer it
    ...
    if user_input:
        ...
        if client:
            try:
                # Use new OpenAI SDK client (OpenAI()) - responses.create style
                ...
                # parse response (modern API returns output list)
                # attempt to extract text in common shapes
                out = ""
                if isinstance(response.output, list) and len(response.output) > 0:
                    # some SDKs put text under .content[0].text
                    # also try fallback to .text if present
                    try:
                        # iterate blocks
                        ...
                        pass
                # fallback: some clients expose response.output_text
                if not out:
        ...
        # After handling spoken param, clear it from URL for clarity
        try:
            if spoken_qp:
